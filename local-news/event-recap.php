...
$canonical_url = 'https://mycovai.in/local-news/event-recap.php?slug=' . urlencode($slug);

// Look for images in /local-events/uploads/recaps/{slug}/
$recapDir = __DIR__ . '/../local-events/uploads/recaps/' . $slug;
$images = [];
if (is_dir($recapDir)) {
  foreach (scandir($recapDir) as $f) {
    if (preg_match('/\.(jpe?g|png|webp)$/i', $f)) {
      $images[] = '/local-events/uploads/recaps/' . $slug . '/' . $f;
    }
  }
}
?>
