...
$stmt = $conn->prepare("
    SELECT jp.id, jp.title, jp.location, jp.status, jp.created_at, jp.salary_range, e.company_name, e.phone
    FROM job_postings jp
    INNER JOIN employers e ON e.id = jp.employer_id
    WHERE jp.title = ? AND jp.location = ?
    ORDER BY jp.id DESC
    LIMIT 1
");
...
$statusCol = $conn->query("SHOW COLUMNS FROM job_postings LIKE 'status'")->fetch_assoc();
if ($statusCol) {
    echo "status_column_type: " . $statusCol['Type'] . PHP_EOL;
}
