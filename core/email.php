<?php
/**
 * Lightweight email helper for MyCovai
 * Uses PHP mail() by default; can be swapped to PHPMailer later.
 */

function sendEmail(string $to, string $subject, string $htmlBody, string $from = 'no-reply@mycovai.in', string $fromName = 'MyCovai') : bool {
...
function renderEmailTemplate(string $title, string $bodyHtml): string {
    return '<!doctype html><html><head><meta charset="utf-8"><title>' .
           ...
           '<p style="font-size:12px;color:#6b7280;margin:0">This is an automated message from MyCovai.</p>' .
           '</div></body></html>';
}
