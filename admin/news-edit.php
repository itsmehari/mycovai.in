...
// Set default values
$title_val = $news['title'] ?? '';
$summary_val = $news['summary'] ?? '';
$date_val = $news['date'] ?? date('Y-m-d');
$tags_val = $news['tags'] ?? '';
$image_val = $news['image'] ?? '';
$article_url_val = $news['article_url'] ?? '';
// Handle update
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title_val && $summary_val && $date_val && $image_val && $article_url_val) {
        $stmt = $conn->prepare('UPDATE news_bulletin SET title=?, summary=?, date=?, tags=?, image=?, article_url=? WHERE id=?');
        $stmt->bind_param('ssssssi', $title_val, $summary_val, $date_val, $tags_val, $image_val, $article_url_val, $id);
        if ($stmt->execute()) {
            $_SESSION['flash_success'] = 'News item updated successfully!';
            header('Location: news-list.php');
            exit;
        } else {
            $error = 'Error updating news item.';
        }
        $stmt->close();
    } else {
...
<div class="container-fluid">
  <div class="row">
    ...
    <main class="col-md-9 ml-sm-auto col-lg-10 px-4 main-content" aria-label="Main content">
      ...
      <h2 class="mb-4">Edit News Bulletin</h2>
      <?php if ($error): ?><div class="alert alert-danger"><?php echo $error; ?></div><?php endif; ?>
      <?php if ($news): ?>
      <form method="POST" autocomplete="off" aria-label="Edit News Bulletin">
        ...
        <div class="form-group">
          ...
          <input type="date" class="form-control" id="date" name="date" value="<?php echo htmlspecialchars($date_val); ?>" required aria-required="true">
        </div>
        <div class="form-group">
