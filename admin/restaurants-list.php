...
// Count total records
$t = covai_table('restaurants');
$count_sql = 'SELECT COUNT(*) FROM `' . $t . '` WHERE 1=1';
if ($search) {
...
$sql = "SELECT id, name, address, locality, cuisine, cost_for_two, rating, availability, imagelocation FROM `" . $t . "` WHERE 1=1";
if ($search) {
...
if (isset($_GET['delete'])) {
    ...
    $del = $conn->prepare('DELETE FROM `' . covai_table('restaurants') . '` WHERE id = ?');
    $del->bind_param('i', $id);
    if ($del->execute()) {
