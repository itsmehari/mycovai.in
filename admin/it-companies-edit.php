...
if ($slno <= 0) { header('Location: featured-it-list.php'); exit; }

// Fetch record
$stmt = $conn->prepare('SELECT slno, company_name, address, industry_type, about, services, careers_url FROM `' . covai_table('it_companies') . '` WHERE slno = ?');
$stmt->bind_param('i', $slno);
$stmt->execute();
$res = $stmt->get_result();
$company = $res ? $res->fetch_assoc() : null;
$stmt->close();
if (!$company) { $_SESSION['flash_error'] = 'Company not found.'; header('Location: featured-it-list.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $upd = $conn->prepare('UPDATE `' . covai_table('it_companies') . '` SET about = ?, services = ?, careers_url = ? WHERE slno = ?');
...
<head>
  ...
  <title>Edit IT Company - MyCovai</title>
...
<div class="container-fluid">
  <div class="row">
    ...
    <main class="col-md-9 ml-sm-auto col-lg-10 px-4 main-content" aria-label="Main content">
      ...
      <h2 class="mb-4">Edit IT Company</h2>
      <div class="card">
        <div class="card-body">
          ...
          <form method="post">
            ...
            <div class="d-flex justify-content-between">
              <a class="btn btn-secondary" href="featured-it-list.php">Back</a>
