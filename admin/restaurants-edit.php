...
// Fetch current data
$stmt = $conn->prepare('SELECT *, ST_X(geolocation) AS lon, ST_Y(geolocation) AS lat FROM `' . covai_table('restaurants') . '` WHERE id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error && $name_val && $address_val && $locality_val && $cuisine_val && $cost_for_two_val && $rating_val && $availability_val && $imagelocation_val) {
        $stmt = $conn->prepare('UPDATE `' . covai_table('restaurants') . '` SET name=?, address=?, locality=?, cuisine=?, cost_for_two=?, rating=?, availability=?, accessibility=?, reviews=?, imagelocation=?, geolocation=ST_GeomFromText(?) WHERE id=?');
...
<head>
    ...
    <title>Edit Restaurant - MyCovai CMS</title>
