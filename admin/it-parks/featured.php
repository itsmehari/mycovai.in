...
require __DIR__ . '/../../directory/data/it-parks-data.php';

// Ensure featured table exists
$tFeat = covai_table('it_parks_feat');
$conn->query("CREATE TABLE IF NOT EXISTS `$tFeat` (
...
$parkIdToName = [];
foreach ((isset($covai_it_parks_all) ? $covai_it_parks_all : $omr_it_parks_all) as $p) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'create' || $action === 'update') {
    ...
    if ($park_id <= 0 || !isset($parkIdToName[$park_id])) {
      ...
      $q = "SELECT id FROM `$tFeat` WHERE rank_position=? AND id<>? AND (COALESCE(start_at, '1970-01-01') <= COALESCE(?, '9999-12-31')) AND (COALESCE(?, '1970-01-01') <= COALESCE(end_at, '9999-12-31')) LIMIT 1";
      $chk = $conn->prepare($q);
      ...
      if ($action === 'create') {
        $stmt = $conn->prepare("INSERT INTO `$tFeat` (park_id, rank_position, blurb, cta_text, cta_url, start_at, end_at) VALUES (?,?,?,?,?,?,?)");
        ...
        if ($id > 0) {
          $stmt = $conn->prepare("UPDATE `$tFeat` SET park_id=?, rank_position=?, blurb=?, cta_text=?, cta_url=?, start_at=?, end_at=? WHERE id=?");
    ...
    $id = isset($_POST['id']) ? (int)$_POST['id'] : 0;
    if ($id > 0) {
      $stmt = $conn->prepare("DELETE FROM `$tFeat` WHERE id=?");
...
// Load rows
$rows = [];
$res = $conn->query("SELECT id, park_id, rank_position, blurb, cta_text, cta_url, start_at, end_at, created_at FROM `$tFeat` ORDER BY rank_position ASC, id DESC");
          ...
          <div class="form-row">
            <div class="form-group col-md-4">
              ...
              <select name="park_id" id="park_id" class="form-control" required>
                ...
                <?php foreach ((isset($covai_it_parks_all) ? $covai_it_parks_all : $omr_it_parks_all) as $p): ?>
