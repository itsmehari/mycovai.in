...
if (isset($_GET['action']) && $_GET['action'] === 'export') {
  ...
  $rs = $conn->query('SELECT id, name, locality, address, phone, website, inauguration_year, owner, built_up_area, total_area, image, lat, lng, verified FROM `' . covai_table('it_parks') . '` ORDER BY id ASC');
  if ($rs) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action']==='import') {
  if (!empty($_FILES['csv']['name']) && is_uploaded_file($_FILES['csv']['tmp_name'])) {
    ...
    if ($fp) {
      ...
      $stmtIns = $conn->prepare('INSERT INTO `' . covai_table('it_parks') . '` (name, locality, address, phone, website, inauguration_year, owner, built_up_area, total_area, image, lat, lng, verified) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)');
      $stmtUpd = $conn->prepare('UPDATE `' . covai_table('it_parks') . '` SET name=?, locality=?, address=?, phone=?, website=?, inauguration_year=?, owner=?, built_up_area=?, total_area=?, image=?, lat=?, lng=?, verified=? WHERE id=?');
