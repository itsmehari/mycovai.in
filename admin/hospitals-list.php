...
if ($search !== '') {
    ...
    $where .= " AND (hospitalname LIKE '{$esc}' OR address LIKE '{$esc}' OR locality LIKE '{$esc}') ";
}
...
$sql = 'SELECT slno, hospitalname, address, locality, verified FROM `' . $t . '`' . $where . ' ORDER BY hospitalname ASC LIMIT ' . $per_page . ' OFFSET ' . $offset;
$res = $conn->query($sql);
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Manage Hospitals - MyCovai</title>
      ...
      <form method="get" class="mb-3">
        <div class="input-group">
          ...
          <div class="input-group-append">
            ...
            <?php if ($search !== ''): ?><a href="hospitals-list.php" class="btn btn-secondary ml-2">Clear</a><?php endif; ?>
          </div>
        </div>
      </form>
      ...
      <div class="table-responsive bg-white p-3">
            ...
            <?php if ($res && $res->num_rows > 0): ?>
              <?php while ($row = $res->fetch_assoc()): ?>
                <tr>
                  ...
                  <td>
                    <strong><?php echo htmlspecialchars($row['hospitalname']); ?></strong><br>
                  ...
                  <td>
                    <a class="btn btn-sm btn-info" href="hospitals-edit.php?slno=<?php echo (int)$row['slno']; ?>">Edit</a>
                    <?php
                      $slugBase = strtolower(preg_replace('/[^a-zA-Z0-9]+/', '-', $row['hospitalname']));
                      $slugBase = trim($slugBase, '-');
                      $pretty = '/hospitals/' . $slugBase . '-' . (int)$row['slno'];
                    ?>
        ...
        <nav aria-label="Pagination"><ul class="pagination">
          <li class="page-item <?php echo $page <= 1 ? 'disabled' : ''; ?>">
            <a class="page-link" href="hospitals-list.php?page=<?php echo $page - 1; ?><?php echo $search !== '' ? '&search=' . urlencode($search) : ''; ?>">Previous</a>
          </li>
          <?php for ($i = 1; $i <= $pages; $i++): ?>
            <li class="page-item <?php echo $i === $page ? 'active' : ''; ?>">
              <a class="page-link" href="hospitals-list.php?page=<?php echo $i; ?><?php echo $search !== '' ? '&search=' . urlencode($search) : ''; ?>"><?php echo $i; ?></a>
            </li>
          <?php endfor; ?>
          <li class="page-item <?php echo $page >= $pages ? 'disabled' : ''; ?>">
            <a class="page-link" href="hospitals-list.php?page=<?php echo $page + 1; ?><?php echo $search !== '' ? '&search=' . urlencode($search) : ''; ?>">Next</a>
          </li>
        </ul></nav>
        <?php endif; ?>
      </div>
