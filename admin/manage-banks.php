...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
  if (!verify_csrf_token($_POST['csrf_token'] ?? '')) { $_SESSION['flash_error'] = 'Invalid CSRF token'; header('Location: manage-banks.php'); exit; }
  $id = (int)($_POST['id'] ?? 0);
  $del = $conn->prepare('DELETE FROM `' . covai_table('banks') . '` WHERE slno = ?');
  $del->bind_param('i', $id);
  $del->execute();
  $del->close();
  $_SESSION['flash_success'] = 'Bank deleted';
  header('Location: manage-banks.php'); exit;
}

$q = isset($_GET['q']) ? trim($_GET['q']) : '';
$sql = 'SELECT slno, bankname, address, contact FROM `' . covai_table('banks') . '`';
...
<head>
  ...
  <title>Manage Banks - Admin</title>
        ...
        <table class="table table-bordered table-hover">
          ...
          <?php if ($res && $res->num_rows > 0): while($row = $res->fetch_assoc()): ?>
            <tr>
              ...
              <td>
                <form method="post" onsubmit="return confirm('Delete this bank?');" class="d-inline">
