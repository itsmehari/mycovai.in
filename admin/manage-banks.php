...
$table = covai_table('banks');
$list_script = 'manage-banks.php';
...
if ($q !== '') {
    ...
    $where = " WHERE bankname LIKE '" . $esc . "' OR address LIKE '" . $esc . "'";
}
...
$sql = 'SELECT slno, bankname, address, contact FROM `' . $table . '`' . $where;
$sql .= ' ORDER BY bankname ASC LIMIT ' . (int) $pagination['limit'] . ' OFFSET ' . (int) $pagination['offset'];
$res = $conn->query($sql);
?>
<!DOCTYPE html>
<html lang="en">
