...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error && $name_val && $address_val && $locality_val && $cuisine_val && $cost_for_two_val && $rating_val && $availability_val && $imagelocation_val) {
        $stmt = $conn->prepare('INSERT INTO `' . covai_table('restaurants') . '` (name, address, locality, cuisine, cost_for_two, rating, availability, accessibility, reviews, imagelocation, geolocation) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ST_GeomFromText(?))');
...
<head>
    ...
    <title><?php echo $title; ?> - MyCovai</title>
