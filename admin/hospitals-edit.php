...
if ($slno <= 0) { header('Location: hospitals-list.php'); exit; }

// Fetch
$stmt = $conn->prepare('SELECT slno, hospitalname, address, locality, about, services, careers_url FROM `' . covai_table('hospitals') . '` WHERE slno = ?');
$stmt->bind_param('i', $slno);
$stmt->execute();
$res = $stmt->get_result();
$row = $res ? $res->fetch_assoc() : null;
$stmt->close();
if (!$row) { $_SESSION['flash_error'] = 'Hospital not found.'; header('Location: hospitals-list.php'); exit; }

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        $_SESSION['flash_error'] = 'Invalid CSRF token.';
        header('Location: hospitals-edit.php?slno=' . $slno);
        exit;
    }
    ...
    $upd = $conn->prepare('UPDATE `' . covai_table('hospitals') . '` SET about = ?, services = ?, careers_url = ? WHERE slno = ?');
...
<head>
  ...
  <title>Edit Hospital - MyCovai</title>
...
<div class="container-fluid">
  <div class="row">
    ...
    <main class="col-md-9 ml-sm-auto col-lg-10 px-4 main-content" aria-label="Main content">
      ...
      <div class="card">
        <div class="card-body">
          <h5 class="card-title mb-3"><?php echo htmlspecialchars($row['hospitalname']); ?></h5>
          ...
          <form method="post">
            ...
            <div class="form-group">
              ...
              <textarea id="about" name="about" class="form-control" rows="5" maxlength="3000" placeholder="Overview, specialties, facilities..."><?php echo htmlspecialchars($row['about'] ?? ''); ?></textarea>
            </div>
            <div class="form-group">
              <label for="services">Services</label>
              <textarea id="services" name="services" class="form-control" rows="4" maxlength="3000" placeholder="Departments, services, hours..."><?php echo htmlspecialchars($row['services'] ?? ''); ?></textarea>
            </div>
            <div class="form-group">
            ...
            <div class="d-flex justify-content-between">
              <a class="btn btn-secondary" href="hospitals-list.php">Back</a>
