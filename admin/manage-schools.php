...
$table = covai_table('schools');
$list_script = 'manage-schools.php';
...
if ($q !== '') {
    ...
    $where = " WHERE schoolname LIKE '" . $esc . "' OR address LIKE '" . $esc . "'";
}
...
$sql = 'SELECT slno, schoolname, address, contact FROM `' . $table . '`' . $where;
$sql .= ' ORDER BY schoolname ASC LIMIT ' . (int) $pagination['limit'] . ' OFFSET ' . (int) $pagination['offset'];
$res = $conn->query($sql);
?>
<!DOCTYPE html>
<html lang="en">
