...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
  if (!verify_csrf_token($_POST['csrf_token'] ?? '')) { $_SESSION['flash_error'] = 'Invalid CSRF token'; header('Location: manage-hospitals.php'); exit; }
  $id = (int)($_POST['id'] ?? 0);
  $del = $conn->prepare('DELETE FROM `' . covai_table('hospitals') . '` WHERE slno = ?');
  $del->bind_param('i', $id);
  $del->execute();
  $del->close();
  $_SESSION['flash_success'] = 'Hospital deleted';
  header('Location: manage-hospitals.php'); exit;
}

$q = isset($_GET['q']) ? trim($_GET['q']) : '';
$sql = 'SELECT slno, hospitalname, address, contact FROM `' . covai_table('hospitals') . '`';
