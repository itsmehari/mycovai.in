...
$t = covai_table('gov_offices');
$count_sql = 'SELECT COUNT(*) FROM `' . $t . '`' . $where;
$count_res = $conn->query($count_sql);
$total = $count_res ? (int)$count_res->fetch_row()[0] : 0;
$pages = max(1, (int)ceil($total / $per_page));

$sql = 'SELECT slno, office_name, address FROM `' . $t . '`' . $where . ' ORDER BY office_name ASC LIMIT ' . $per_page . ' OFFSET ' . $offset;
$res = $conn->query($sql);
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Manage Government Offices - MyCovai</title>
