...
$tFeat = covai_table('it_companies_feat');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'approve') {
    ...
    $tComp = covai_table('it_companies');
$ins = $conn->prepare('INSERT INTO `' . $tComp . '` (company_name, address, locality, contact, industry_type, verified) VALUES (?, ?, ?, ?, ?, 1)');
    ...
    if ($ins->execute()) {
        ...
        $updSlug = $conn->prepare('UPDATE `' . covai_table('it_companies') . '` SET slug = ? WHERE slno = ?');
        ...
        if ($feature) {
            ...
            $feat = $conn->prepare('INSERT INTO `' . covai_table('it_companies_feat') . '` (company_slno, rank_position, blurb, cta_text, cta_url, start_at, end_at) VALUES (?, ?, ?, ?, ?, ?, ?)');
...
<head>
  ...
  <title>IT Company Submissions - MyCovai CMS</title>
