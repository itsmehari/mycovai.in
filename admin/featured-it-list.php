...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_id'])) {
    ...
    $upd = $conn->prepare('UPDATE `' . covai_table('it_companies_feat') . '` SET rank_position=?, blurb=?, cta_text=?, cta_url=?, start_at=?, end_at=? WHERE id=?');
...
$sql = "SELECT f.id, f.rank_position, f.blurb, f.cta_text, f.cta_url, f.start_at, f.end_at, c.slno, c.company_name
        FROM `$tFeat` f
        JOIN `$tComp` c ON c.slno = f.company_slno
...
<head>
  ...
  <title>Manage Featured IT Companies - MyCovai</title>
