...
$urls = [];
// Listing page
$urls[] = 'https://mycovai.in/local-events/';
// Date intent hubs
$urls[] = 'https://mycovai.in/local-events/today';
$urls[] = 'https://mycovai.in/local-events/weekend';
$urls[] = 'https://mycovai.in/local-events/month';
...
try {
  ...
  if ($conn && !$conn->connect_error) {
    ...
    while ($row = $res->fetch_assoc()) {
      $urls[] = 'https://mycovai.in/local-events/event/' . urlencode($row['slug']);
    }

    // Category hubs
    $res = $conn->query("SELECT slug FROM event_categories WHERE is_active = 1 ORDER BY display_order, name LIMIT 100");
    while ($row = $res->fetch_assoc()) {
      $urls[] = 'https://mycovai.in/local-events/category/' . urlencode($row['slug']);
    }
    ...
    while ($row = $res->fetch_assoc()) {
      ...
      $urls[] = 'https://mycovai.in/local-events/locality/' . urlencode($slug);
    }
    ...
    while ($row = $res->fetch_assoc()) {
      ...
      $urls[] = 'https://mycovai.in/local-events/venue/' . urlencode($vslug);
    }
  }
} catch (Throwable $e) {
