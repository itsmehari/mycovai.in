...
<head>
  ...
  <title><?php echo $event ? htmlspecialchars($event['title']) . ' – ' . htmlspecialchars(defined('SITE_NAME') ? SITE_NAME : 'MyOMR') : 'Event – ' . htmlspecialchars(defined('SITE_NAME') ? SITE_NAME : 'MyOMR'); ?></title>
  <meta name="description" content="<?php echo $event ? htmlspecialchars(substr(strip_tags($event['description']),0,160)) : 'Event details'; ?>" />
  <?php if ($event): ?>
  <link rel="canonical" href="<?php echo 'https://mycovai.in/local-events/event/' . urlencode($slug); ?>" />
  <!-- Open Graph / Twitter for Event -->
  <meta property="og:title" content="<?php echo htmlspecialchars($event['title'] . ' – ' . (defined('SITE_NAME') ? SITE_NAME : 'MyOMR')); ?>" />
  <meta property="og:description" content="<?php echo htmlspecialchars(substr(strip_tags($event['description']),0,160)); ?>" />
  <meta property="og:url" content="<?php echo 'https://mycovai.in/local-events/event/' . urlencode($slug); ?>" />
  <meta property="og:type" content="event" />
  <meta property="og:image" content="<?php echo htmlspecialchars($event['image_url'] ?: 'https://mycovai.in/My-OMR-Logo.jpg'); ?>" />
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:title" content="<?php echo htmlspecialchars($event['title'] . ' – ' . (defined('SITE_NAME') ? SITE_NAME : 'MyOMR')); ?>" />
  <meta name="twitter:description" content="<?php echo htmlspecialchars(substr(strip_tags($event['description']),0,160)); ?>" />
  <meta name="twitter:image" content="<?php echo htmlspecialchars($event['image_url'] ?: 'https://mycovai.in/My-OMR-Logo.jpg'); ?>" />
  <?php endif; ?>
  ...
  <?php if ($event): 
    $detailLd = [
      ...
      'location' => [
        ...
        'address' => [
          '@type' => 'PostalAddress',
          'addressLocality' => $event['locality'] ?: (defined('SITE_REGION_SHORT') && defined('SITE_REGION') ? SITE_REGION_SHORT . ', ' . SITE_REGION : 'OMR, Chennai'),
          'addressRegion' => 'Tamil Nadu',
          'addressCountry' => 'IN'
        ]
      ],
    'url' => 'https://mycovai.in/local-events/event/' . urlencode($slug)
    ];
    ...
    if (!$event['is_free'] && !empty($event['price'])) {
      $detailLd['offers'] = [
        ...
        'price' => $event['price'],
        'priceCurrency' => 'INR',
        'url' => 'https://mycovai.in/local-events/event-detail-omr.php?slug=' . urlencode($slug)
      ];
    }
  ?>
    ...
    <?php else: ?>
      ...
      <div class="d-flex flex-wrap gap-2">
        ...
        <?php endif; ?>
              ...
              <h5>Share</h5>
              <?php 
                $baseUrl = 'https://mycovai.in/local-events/event-detail-omr.php?slug=' . urlencode($slug);
