...
if (!empty($employerEmail)) {
    ...
    if ($emailResult && $emailResult->num_rows > 0) {
        ...
        if ($employerId !== $correctId) {
            ...
            error_log("my-posted-jobs-covai.php: Updated employer_id from session to {$correctId} based on email");
        }
    }
}
...
if ($employerId > 0) {
    ...
    error_log("my-posted-jobs-covai.php: Executing query: " . $jobsQuery);
    $jobsResult = $conn->query($jobsQuery);
    
    if ($jobsResult) {
        if ($jobsResult->num_rows > 0) {
            $jobs = $jobsResult->fetch_all(MYSQLI_ASSOC);
            error_log("my-posted-jobs-covai.php: Found " . count($jobs) . " jobs for employer_id = {$employerId}");
        } else {
            error_log("my-posted-jobs-covai.php: Query returned 0 rows for employer_id = {$employerId}");
        }
    } else {
        error_log("my-posted-jobs-covai.php: Query failed: " . $conn->error);
    }
    ...
    if (empty($jobs) && !empty($employerEmail)) {
        ...
        error_log("my-posted-jobs-covai.php: Trying fallback query by email");
        $checkResult = $conn->query($checkQuery);
        if ($checkResult && $checkResult->num_rows > 0) {
            $jobs = $checkResult->fetch_all(MYSQLI_ASSOC);
            error_log("my-posted-jobs-covai.php: Found " . count($jobs) . " jobs by email lookup");
        } else {
            error_log("my-posted-jobs-covai.php: Fallback query also returned 0 rows");
        }
    }
    ...
    if (empty($jobs)) {
        ...
        if ($flexibleResult && $flexibleResult->num_rows > 0) {
            ...
            error_log("my-posted-jobs-covai.php: Found " . count($jobs) . " jobs using flexible query");
        }
    }
} else {
    error_log("my-posted-jobs-covai.php: Invalid employer_id = {$employerId}");
}
...
<head>
    ...
    <title>My Posted Jobs - Employer Dashboard | MyCovai</title>
    ...
    <div class="container">
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3 text-white">
            ...
            <div class="d-flex gap-2 flex-wrap">
                <a class="btn btn-light" href="employer-dashboard-covai.php"><i class="fas fa-users me-2"></i>View All Applications</a>
                <a class="btn btn-outline-light" href="post-job-covai.php"><i class="fas fa-plus me-2"></i>Post New Job</a>
                <a class="btn btn-outline-light" href="employer-logout-covai.php"><i class="fas fa-right-from-bracket me-2"></i>Logout</a>
            </div>
        </div>
    </div>
</section>
    ...
    <div class="container">
            ...
            <div class="col-12">
                <div class="card-modern p-4">
                    <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                        ...
                        <div class="d-flex gap-2 flex-wrap">
                            <a href="post-job-covai.php" class="btn btn-success">
                                <i class="fas fa-plus me-2"></i>Post New Job
                            </a>
                            <a href="edit-employer-profile-covai.php" class="btn btn-outline-primary">
        ...
        <?php if (empty($jobs)): ?>
            <div class="card-modern p-5 text-center">
                ...
                <p class="text-muted mb-4">Start reaching candidates in Coimbatore by creating your first job listing.</p>
                <a href="post-job-covai.php" class="btn btn-primary"><i class="fas fa-plus me-2"></i>Post your first job</a>
            </div>
        <?php else: ?>
                    ...
                    <table class="table table-hover align-middle mb-0 dashboard-table">
                        ...
                        <tbody>
                            <?php foreach ($jobs as $job): ?>
                                ...
                                <tr>
                                    <td>
                                        <a href="<?php echo htmlspecialchars(getJobDetailUrl($job['id'], $job['title'] ?? '')); ?>" class="fw-semibold text-decoration-none" target="_blank">
                                    ...
                                    <td class="text-end dashboard-actions">
                                        <div class="btn-group" role="group" aria-label="Manage job">
                                            <a href="edit-job-covai.php?id=<?php echo (int)$job['id']; ?>" class="btn btn-sm btn-outline-primary" title="Edit job">
                                                <i class="fas fa-pen"></i>
                                            </a>
                                            <a href="view-applications-covai.php?id=<?php echo (int)$job['id']; ?>" class="btn btn-sm btn-outline-secondary" title="View applications">
                                                <i class="fas fa-inbox"></i>
                                            </a>
                                            <a href="<?php echo htmlspecialchars(getJobDetailUrl($job['id'], $job['title'] ?? '')); ?>" class="btn btn-sm btn-outline-success" title="Preview listing" target="_blank">
