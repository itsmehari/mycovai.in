...
function generateJobPostingSchema($job) {
    ...
    $salary = parseSalaryRangeForSchema($job['salary_range'] ?? '');
    ...
    if ($salary) {
        ...
        if (isset($salary['value'])) {
            $salaryValue['value'] = $salary['value'];
        } else {
            $salaryValue['minValue'] = $salary['minValue'];
            $salaryValue['maxValue'] = $salary['maxValue'];
        }
