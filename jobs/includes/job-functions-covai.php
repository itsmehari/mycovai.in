<?php
/**
 * Job Portal Helper Functions (MyCovai / Coimbatore)
 *
...
/**
 * Social profile URLs for JSON-LD sameAs (empty when not configured).
 *
 * @return list<string>
 */
function covai_jobs_schema_same_as_array(): array {
    $urls = [];
    foreach ([
...
/**
 * Paginated approved jobs for the public index (filters in SQL).
 *
 * @return array{jobs: array<int, array<string, mixed>>, total: int}
 */
function covai_jobs_index_fetch(mysqli $conn, array $filters, int $page, int $perPage): array {
    $page = max(1, $page);
    $perPage = max(1, min(100, $perPage));
    $offset = ($page - 1) * $perPage;

    $where = ["LOWER(TRIM(COALESCE(j.status, ''))) = 'approved'"];
    $types = '';
    $params = [];
    ...
    if (!empty($filters['search'])) {
        $where[] = '(j.title LIKE ? OR j.description LIKE ? OR COALESCE(e.company_name, \'\') LIKE ?)';
        $types .= 'sss';
        $term = '%' . $filters['search'] . '%';
        $params[] = $term;
        $params[] = $term;
    ...
    $countSql = "SELECT COUNT(*) AS c FROM job_postings j LEFT JOIN employers e ON j.employer_id = e.id WHERE {$whereSql}";
    ...
    $total = ($countRes && ($row = $countRes->fetch_assoc())) ? (int) $row['c'] : 0;
    $stmt->close();

    $typesMain = $types . 'ii';
    $paramsMain = array_merge($params, [$perPage, $offset]);

    $selectSql = "SELECT j.*, e.company_name, e.contact_person, e.email AS employer_email,
...
/**
 * Build SEO-friendly job detail URL.
 *
 * Pattern: /jobs/{title-slug}-{id}
 *
 * @param int|string $jobId
 * @param string $title
 * @return string
 */
function getJobDetailUrl($jobId, $title = '') {
    $id = (int) $jobId;
    if ($id <= 0) {
        return '/jobs/';
    }

    $slug = createSlug((string) $title);
    if ($slug === '') {
        return '/jobs/' . $id;
    }
