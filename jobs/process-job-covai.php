<?php
/**
 * MyCovai Job Portal – Process job posting
 *
...
if ($success) {
    if ($is_update) {
        ...
        header('Location: my-posted-jobs-covai.php?updated=1');
    } else {
        // Success page for new posting
        header('Location: job-posted-success-covai.php?id=' . urlencode($job_id));
    }
    exit;
} else {
    ...
    if ($is_update) {
        header('Location: edit-job-covai.php?id=' . urlencode($job_id) . '&error=1');
    } else {
        header('Location: post-job-covai.php?error=1');
    }
    exit;
}
?>
