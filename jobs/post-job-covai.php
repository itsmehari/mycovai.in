<?php
/**
 * MyCovai Job Portal – Post job form
 *
...
try {
    ...
    if (!is_array($categories)) {
        $categories = []; // Fallback to empty array if function fails
        if (defined('DEVELOPMENT_MODE') && DEVELOPMENT_MODE) {
            error_log("getJobCategories() returned non-array: " . gettype($categories));
        }
    }
    
    // Debug: Log categories count in development mode
    if (defined('DEVELOPMENT_MODE') && DEVELOPMENT_MODE) {
        error_log("Loaded " . count($categories) . " job categories in post-job-covai.php");
        if (count($categories) === 0) {
            error_log("WARNING: No categories found. Testing direct query...");
            // Try direct query as fallback
            global $conn;
            if (isset($conn) && $conn) {
                // Try with is_active first, then without
                $direct_result = $conn->query("SELECT * FROM job_categories WHERE is_active = 1 ORDER BY name");
                if (!$direct_result || $direct_result->num_rows === 0) {
                    $direct_result = $conn->query("SELECT * FROM job_categories ORDER BY name");
                }
                if ($direct_result && $direct_result->num_rows > 0) {
                    error_log("Direct query found " . $direct_result->num_rows . " categories - function may have issue");
                    // Try to use direct result
                    $categories = [];
                    while ($row = $direct_result->fetch_assoc()) {
                        $categories[] = $row;
                    }
                    error_log("Using direct query results: " . count($categories) . " categories");
                }
            }
        }
    }
    ...
    if (defined('DEVELOPMENT_MODE') && DEVELOPMENT_MODE) {
        ...
        echo "<strong>Error getting job categories:</strong> " . htmlspecialchars($e->getMessage());
        echo "<br><strong>Stack trace:</strong><pre>" . htmlspecialchars($e->getTraceAsString()) . "</pre>";
        echo "</div>";
    }
    ...
    if (defined('DEVELOPMENT_MODE') && DEVELOPMENT_MODE) {
        echo "<div style='background:#fee; border:2px solid #f00; padding:15px; margin:10px; border-radius:5px;'>";
        echo "<strong>Fatal error getting job categories:</strong> " . htmlspecialchars($e->getMessage());
        echo "<br><strong>File:</strong> " . htmlspecialchars($e->getFile()) . ":" . $e->getLine();
        echo "</div>";
    }
    error_log("Fatal error getting job categories: " . $e->getMessage() . " in " . $e->getFile() . ":" . $e->getLine());
    $categories = []; // Fallback to empty array
}
...
$page_title = "Post a Job in " . $region_full . " | " . $site_name;
$page_description = "Post your job vacancy on " . $site_name . ". Reach qualified candidates in " . $region_full . ". Free job listings for employers.";
$canonical_url = (defined('SITE_CANONICAL_BASE') ? SITE_CANONICAL_BASE : 'https://mycovai.in') . '/jobs/post-job-covai.php';
...
<head>
    ...
    <meta name="keywords" content="post job Coimbatore, job vacancy Coimbatore, hire employees Covai, job listing Coimbatore">
        ...
        <div class="container">
            <div class="row align-items-center">
                <div class="col-lg-8">
                    ...
                    <p class="mb-0">Reach 1000+ qualified candidates in Coimbatore</p>
                </div>
    ...
    <section class="py-5 bg-light">
        <div class="container">
            ...
            <?php if (empty($categories)): ?>
                <div class="alert alert-warning" role="alert" style="margin-bottom: 2rem; padding: 1rem; border-radius: 8px; background: #fff3cd; border: 1px solid #ffc107;">
                    <strong><i class="fas fa-exclamation-triangle"></i> Warning:</strong> No job categories found in the database. 
                    <ul style="margin: 10px 0; padding-left: 20px;">
                        <li>Categories exist in the database but <code>is_active</code> might be 0 or NULL for all records</li>
                        <li><strong>Quick Fix:</strong> Run this SQL in phpMyAdmin: <code>UPDATE job_categories SET is_active = 1;</code></li>
                        <li>Or download and run <code>FIX-CATEGORIES.sql</code> from the job-listings folder</li>
                        <li>After fixing, refresh this page - categories should appear immediately</li>
                    </ul>
                    <?php if (defined('DEVELOPMENT_MODE') && DEVELOPMENT_MODE): ?>
                        <br><small><strong>Debug info:</strong> Categories array is empty. Check if <code>job_categories</code> table exists and contains records with <code>is_active = 1</code>.</small>
                    <?php endif; ?>
                </div>
            <?php elseif (defined('DEVELOPMENT_MODE') && DEVELOPMENT_MODE): ?>
                <!-- Debug info in dev mode -->
                <div style="margin-bottom: 1rem; padding: 0.5rem; background: #d1ecf1; border-radius: 4px; font-size: 0.85rem;">
                    ✅ Loaded <?php echo count($categories); ?> categories successfully
                </div>
            <?php endif; ?>
            
            <form method="POST" action="process-job-covai.php" id="post-job-form" class="needs-validation" novalidate>
                    ...
                    <div class="form-section form-section-column">
                        ...
                        <div class="col-md-12">
                            <div class="form-group-modern">
                                ...
                                <input type="text" class="form-control-modern" id="company_name" name="company_name" required placeholder="Enter your company name">
                        ...
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                ...
                                <input type="text" class="form-control-modern" id="contact_person" name="contact_person" required placeholder="Your name">
                        ...
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                ...
                                <input type="tel" class="form-control-modern" id="phone" name="phone" required pattern="[\d\s\-\+\(\)]+" minlength="10" maxlength="15" placeholder="+91 9876543210">
                        ...
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                ...
                                <input type="email" class="form-control-modern" id="email" name="email" required placeholder="contact@company.com">
                        ...
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                ...
                                <input type="url" class="form-control-modern" id="website" name="website" placeholder="https://example.com">
                        ...
                        <div class="col-md-12">
                            <div class="form-group-modern">
                                ...
                                <textarea class="form-control-modern" id="address" name="address" rows="2" placeholder="Enter company address"></textarea>
                            </div>
                        </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section form-section-column">
                        ...
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                ...
                                <input type="text" class="form-control-modern" id="location" name="location" required placeholder="e.g., Gandhipuram, RS Puram, Coimbatore">
                                <div class="help-text-modern"><i class="fas fa-info-circle"></i> Specific location in Coimbatore</div>
                    ...
                    <div class="form-group-modern">
                        <div class="form-check-modern d-flex align-items-start">
                            ...
                            <label class="form-check-label-modern" for="terms">
                                I agree to the <a href="/terms-and-conditions.php" target="_blank" class="text-primary">Terms and Conditions</a> and confirm that this job posting is accurate
                            </label>
