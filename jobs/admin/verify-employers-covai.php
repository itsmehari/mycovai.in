    ...
    if ($employer_id > 0 && in_array($action, ['verify', 'suspend', 'pending'], true)) {
        ...
        if ($info && !empty($info['email'])) {
            ...
            $body = renderEmailTemplate(
                ...
                '<p>Your employer account status on MyCovai has been updated to <strong>' . ucfirst($newStatus) . '</strong>.</p>' .
        ...
        header('Location: verify-employers-covai.php?success=1');
        exit;
    }
}
...
<head>
    ...
    <title>Verify Employers – Admin | MyCovai</title>
