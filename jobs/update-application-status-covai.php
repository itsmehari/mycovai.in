...
if ($application_id <= 0 || empty($status) || $job_id <= 0) {
    header('Location: view-applications-covai.php?id=' . $job_id . '&error=invalid');
    exit;
}
...
if ($verifyResult && $verifyResult->num_rows > 0) {
    ...
    if ($conn->query($updateQuery)) {
        ...
        header('Location: view-applications-covai.php?id=' . $job_id . '&error=update_failed');
    }
} else {
    header('Location: view-applications-covai.php?id=' . $job_id . '&error=unauthorized');
}
exit;
