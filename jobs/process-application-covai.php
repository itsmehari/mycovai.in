<?php
/**
 * MyCovai Job Portal – Process application
 *
...
if ($success) {
    ...
    header('Location: application-submitted-covai.php?job_id=' . urlencode($job_id) . '&applicant=' . urlencode($applicant_name));
    exit;
} else {
    // Error - redirect back to job detail with errors
    $_SESSION['application_errors'] = $errors;
    header('Location: ' . getJobDetailUrl($job_id) . '?error=1');
    exit;
}
?>
