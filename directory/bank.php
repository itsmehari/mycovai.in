...
$stmt = $conn->prepare('SELECT slno, bankname, address, contact, landmark, website FROM `'.covai_table('banks').'` WHERE slno = ?');
...
$canonical = 'https://mycovai.in/banks/' . $slug;
?>
<!DOCTYPE html>
<html lang="en">
<head>
<?php $breadcrumbs = [ ['https://mycovai.in/','Home'], ['https://mycovai.in/banks','Banks'], [$canonical, $name] ]; ?>
<?php include '../components/meta.php'; ?>
<?php include '../components/analytics.php'; ?>
<?php include '../components/head-resources.php'; ?>
<title><?php echo htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?> | Bank in Coimbatore | MyCovai</title>
<link rel="canonical" href="<?php echo htmlspecialchars($canonical, ENT_QUOTES, 'UTF-8'); ?>" />
<meta name="description" content="<?php echo htmlspecialchars($name . ' - Bank in Coimbatore. Address: ' . $address, ENT_QUOTES, 'UTF-8'); ?>" />
...
<div id="main-content" class="container maxw-1280">
  ...
  if ($detectedLocality !== '') {
    ...
    $rel = $conn->prepare('SELECT slno, bankname, address FROM `'.covai_table('banks').'` WHERE slno <> ? AND address LIKE ? ORDER BY bankname ASC LIMIT 6');
