...
$stmt = $conn->prepare('SELECT slno, company_name, address, contact, industry_type, verified, about, services, careers_url FROM `'.covai_table('it-companies').'` WHERE slno = ?');
...
$canonical = 'https://mycovai.in/it-companies/' . $slug;
?>
<!DOCTYPE html>
<html lang="en">
<head>
<?php $breadcrumbs = [
  ...
  ['https://mycovai.in/it-companies','IT Companies'],
  [$canonical, $companyName]
]; ?>
<?php include '../components/meta.php'; ?>
<?php include '../components/analytics.php'; ?>
<?php include '../components/head-resources.php'; ?>
<title><?php echo htmlspecialchars($companyName, ENT_QUOTES, 'UTF-8'); ?> | IT Company in Coimbatore | MyCovai</title>
<link rel="canonical" href="<?php echo htmlspecialchars($canonical, ENT_QUOTES, 'UTF-8'); ?>" />
<meta name="description" content="<?php echo htmlspecialchars($companyName . ' - IT company in Coimbatore. Address: ' . $address, ENT_QUOTES, 'UTF-8'); ?>" />
...
<div id="main-content" class="container" style="max-width:1280px;">
  ...
  <div class="mb-3">
    ...
    <a id="btnEnquire" data-company-id="<?php echo (int)$id; ?>" data-company-name="<?php echo htmlspecialchars($companyName, ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-warning" href="/contact.php?subject=<?php echo urlencode('Listing Enquiry: ' . $companyName); ?>">Enquire</a>
  </div>
  ...
  if ($detectedLocality !== '') {
    ...
    echo '<p><a class="btn btn-sm btn-success" href="/directory/locality/' . htmlspecialchars($hubSlug, ENT_QUOTES, 'UTF-8') . '.php">Visit ' . htmlspecialchars($detectedLocality, ENT_QUOTES, 'UTF-8') . ' hub</a></p>';
  }
  if ($detectedLocality !== '') {
    $like = '%' . $detectedLocality . '%';
    $rel = $conn->prepare('SELECT slno, company_name, address FROM `'.covai_table('it-companies').'` WHERE slno <> ? AND address LIKE ? ORDER BY company_name ASC LIMIT 6');
