...
$stmt = $conn->prepare('SELECT id, name, address, locality, cuisine, cost_for_two, rating, availability, reviews, imagelocation FROM `'.covai_table('restaurants').'` WHERE id = ?');
...
$canonical = 'https://mycovai.in/restaurants/' . $slug;
?>
<!DOCTYPE html>
<html lang="en">
<head>
<?php $breadcrumbs = [
  ...
  ['https://mycovai.in/restaurants','Restaurants'],
  [$canonical, $name]
]; ?>
<?php include '../components/meta.php'; ?>
<?php include '../components/analytics.php'; ?>
<?php include '../components/head-resources.php'; ?>
<title><?php echo htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?> | Restaurant in Coimbatore | MyCovai</title>
<link rel="canonical" href="<?php echo htmlspecialchars($canonical, ENT_QUOTES, 'UTF-8'); ?>" />
<meta name="description" content="<?php echo htmlspecialchars($name . ' - restaurant in Coimbatore. Address: ' . $address, ENT_QUOTES, 'UTF-8'); ?>" />
</head>
<body>
...
<div id="main-content" class="container maxw-1280">
  ...
  <div class="py-3">
    <?php covai_monetized_slot('detail-mid', '336x280'); ?>
  </div>
  ...
  <div class="row">
    ...
    <div class="col-md-4">
      <?php if (!empty($imageUrl)): ?>
        <img src="<?php echo htmlspecialchars($imageUrl); ?>" alt="<?php echo htmlspecialchars($name); ?>" class="img-fluid rounded mb-3" />
      <?php endif; ?>
  ...
  <?php
    if (!empty($locality)) {
      $like = '%' . $locality . '%';
      $rel = $conn->prepare('SELECT id, name, address FROM `'.covai_table('restaurants').'` WHERE id <> ? AND locality LIKE ? ORDER BY rating DESC LIMIT 6');
