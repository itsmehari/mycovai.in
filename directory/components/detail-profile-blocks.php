...
switch (strtolower($detectedLocality)) {
  case 'rs puram':
  case 'rs-puram':
    $nearbyTransit = ['Gandhipuram Bus Stand', 'RS Puram Town Bus']; break;
  case 'gandhipuram':
    $nearbyTransit = ['Gandhipuram Bus Stand', 'Coimbatore Central']; break;
  case 'peelamedu':
    $nearbyTransit = ['Peelamedu Bus Stop', 'Coimbatore Airport']; break;
  case 'saravanampatti':
    $nearbyTransit = ['Saravanampatti Bus Stop', 'Tidel Park Route']; break;
  case 'saibaba colony':
    $nearbyTransit = ['Saibaba Colony Bus Stop', '100 Feet Road']; break;
  default:
    $nearbyTransit = ['Coimbatore City Bus', 'Mettupalayam Road / Trichy Road'];
}
      ...
      <p>
        <?php
          if (is_string($aboutTextDb) && trim($aboutTextDb) !== '') {
            ...
            $fallbackText = !empty($fallback) ? implode(', ', $fallback) : 'Local listing in Coimbatore';
            echo $fallbackText . '.';
          }
        ?>
      </p>
