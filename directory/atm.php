...
$stmt = $conn->prepare('SELECT slno, bankname, address, contact, landmark FROM `'.covai_table('atms').'` WHERE slno = ?');
...
if (!$item) { http_response_code(404); echo '<!DOCTYPE html><html><body><h1>ATM not found</h1></body></html>'; exit; }

$name = $item['bankname'];
$address = $item['address'];
$contact = $item['contact'];
$canonical = 'https://mycovai.in/atms/' . $slug;
?>
<!DOCTYPE html>
<html lang="en">
<head>
<?php $breadcrumbs = [ ['https://mycovai.in/','Home'], ['https://mycovai.in/atms','ATMs'], [$canonical, $name] ]; ?>
<?php include '../components/meta.php'; ?>
<?php include '../components/analytics.php'; ?>
<?php include '../components/head-resources.php'; ?>
<title><?php echo htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?> | ATM in Coimbatore | MyCovai</title>
<link rel="canonical" href="<?php echo htmlspecialchars($canonical, ENT_QUOTES, 'UTF-8'); ?>" />
<meta name="description" content="<?php echo htmlspecialchars($name . ' ATM - Address: ' . $address, ENT_QUOTES, 'UTF-8'); ?>" />
...
<div id="main-content" class="container" style="max-width:1280px;">
  ...
  if ($detectedLocality !== '') {
    ...
    $rel = $conn->prepare('SELECT slno, bankname, address FROM `'.covai_table('atms').'` WHERE slno <> ? AND address LIKE ? ORDER BY bankname ASC LIMIT 6');
