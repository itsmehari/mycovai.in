...
$title = 'Get Your IT Company Listed on MyCovai';
$success = '';
$error = '';

$allowedLocalities = defined('COIMBATORE_LOCALITIES') ? COIMBATORE_LOCALITIES : ['RS Puram','Gandhipuram','Peelamedu','Saravanampatti','Saibaba Colony','Race Course'];
  ...
  if (!$csrf_ok || $honeypot !== '' || !$rate_ok) {
    ...
    if ($company_name === '' || $address === '' || ($email === '' && $phone === '')) {
      ...
      if ($stmt->execute()) {
        ...
        // Email notify
        $to = defined('CONTACT_EMAIL') ? CONTACT_EMAIL : 'info@mycovai.in';
        $subject = 'New IT Company Listing Submission - MyCovai';
        $body = "Company: {$company_name}\nAddress: {$address}\nLocality: {$locality}\nWebsite: {$website}\nContact: {$contact_name}\nEmail: {$email}\nPhone: {$phone}\nIndustry: {$industry_type}\nFeatured? " . ($featured_requested ? 'Yes' : 'No') . "\n\nMessage:\n{$message}";
        @mail($to, $subject, $body, 'From: noreply@mycovai.in');
...
<?php $breadcrumbs = [
  ...
  ['https://mycovai.in/directory/get-listed.php','Get Listed']
]; ?>
...
<div class="container" style="max-width:1280px;">
  ...
  <form id="get-listed-form" method="post" action="">
        ...
        <div class="card h-100">
          <div class="card-body">
            ...
            <div class="form-check">
              ...
              <label class="form-check-label" for="tier_free">Select Free</label>
            </div>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="card h-100 border-success">
          <div class="card-body">
            ...
            <ul class="mb-2">
              <li>Verified badge on listing</li>
              <li>Priority in locality filters</li>
            </ul>
            <div class="form-check">
              ...
              <label class="form-check-label" for="tier_verified">Select Verified</label>
            </div>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="card h-100 border-warning">
          <div class="card-body">
            ...
            <ul class="mb-2">
              <li>Featured card placement</li>
              <li>Custom CTA and blurb</li>
            </ul>
            <div class="form-check">
              ...
              <label class="form-check-label" for="tier_featured">Select Featured</label>
            </div>
            <div class="mt-2">
              <label class="form-label">Logo URL (optional)</label>
              <input type="url" class="form-control" name="logo_url" value="<?php echo htmlspecialchars($_POST['logo_url'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" placeholder="https://...">
            </div>
          </div>
        </div>
      </div>
    ...
    <a href="/directory/it-companies.php" class="btn btn-link">Back to IT companies</a>
  </form>
