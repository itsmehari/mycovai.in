...
$stmt = $conn->prepare('SELECT slno, office_name, address, contact, landmark FROM `'.covai_table('government-offices').'` WHERE slno = ?');
...
$canonical = 'https://mycovai.in/government-offices/' . $slug;
?>
<!DOCTYPE html>
<html lang="en">
<head>
<?php $breadcrumbs = [ ['https://mycovai.in/','Home'], ['https://mycovai.in/government-offices','Government Offices'], [$canonical, $name] ]; ?>
...
<div id="main-content" class="container maxw-1280">
  ...
  if ($detectedLocality !== '') {
    ...
    $rel = $conn->prepare('SELECT slno, office_name, address FROM `'.covai_table('government-offices').'` WHERE slno <> ? AND address LIKE ? ORDER BY office_name ASC LIMIT 6');
