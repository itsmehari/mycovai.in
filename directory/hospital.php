...
$stmt = $conn->prepare('SELECT slno, hospitalname, address, contact, landmark, locality, verified, about, services, careers_url FROM `'.covai_table('hospitals').'` WHERE slno = ?');
...
$canonical = 'https://mycovai.in/hospitals/' . $slug;
?>
<!DOCTYPE html>
<html lang="en">
<head>
<?php $breadcrumbs = [
  ...
  ['https://mycovai.in/hospitals','Hospitals'],
  [$canonical, $name]
]; ?>
...
<div class="container maxw-1280">
  ...
  if ($detectedLocality !== '') {
    ...
    $rel = $conn->prepare('SELECT slno, hospitalname, address FROM `'.covai_table('hospitals').'` WHERE slno <> ? AND address LIKE ? ORDER BY hospitalname ASC LIMIT 6');
...
$stmt = $conn->prepare('SELECT slno, hospitalname, address, contact, landmark FROM `'.covai_table('hospitals').'` WHERE slno = ?');
$stmt->bind_param('i', $id);
$stmt->execute();
$res = $stmt->get_result();
...
<html lang="en">
<head>
<?php $breadcrumbs = [ ['https://mycovai.in/','Home'], ['https://mycovai.in/hospitals','Hospitals'], [$canonical, $name] ]; ?>
<?php include '../components/meta.php'; ?>
<?php include '../components/analytics.php'; ?>
<?php include '../components/head-resources.php'; ?>
<title><?php echo htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?> | Hospital in Coimbatore | MyCovai</title>
