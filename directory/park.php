...
$stmt = $conn->prepare('SELECT slno, parkname, location, area, features, timings FROM `'.covai_table('parks').'` WHERE slno = ?');
...
if (!$row) {
    http_response_code(404);
    echo '<!DOCTYPE html><html><body><h1>Park not found</h1></body></html>';
    exit;
}
...
$canonical = 'https://mycovai.in/parks/' . $slug;
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title><?php echo htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?> | Park in Coimbatore | MyCovai</title>
<link rel="canonical" href="<?php echo htmlspecialchars($canonical, ENT_QUOTES, 'UTF-8'); ?>" />
<meta name="description" content="<?php echo htmlspecialchars($name . ' - park in Coimbatore. Location: ' . $location, ENT_QUOTES, 'UTF-8'); ?>" />
</head>
<body>
...
if (!$item) { http_response_code(404); echo '<!DOCTYPE html><html><body><h1>Park not found</h1></body></html>'; exit; }
...
<html lang="en">
<head>
<?php $breadcrumbs = [ ['https://mycovai.in/','Home'], ['https://mycovai.in/parks','Parks'], [$canonical, $name] ]; ?>
<?php include '../components/meta.php'; ?>
<?php include '../components/analytics.php'; ?>
<?php include '../components/head-resources.php'; ?>
<title><?php echo htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?> | Park in Coimbatore | MyCovai</title>
<link rel="canonical" href="<?php echo htmlspecialchars($canonical, ENT_QUOTES, 'UTF-8'); ?>" />
<meta name="description" content="<?php echo htmlspecialchars($name . ' - Park in Coimbatore. Address: ' . $address, ENT_QUOTES, 'UTF-8'); ?>" />
...
<div id="main-content" class="container maxw-1280">
  ...
  if ($detectedLocality !== '') {
    ...
    $rel = $conn->prepare('SELECT slno, parkname, location FROM `'.covai_table('parks').'` WHERE slno <> ? AND location LIKE ? ORDER BY parkname ASC LIMIT 6');
