...
$sql = "SELECT slno, industry_name, address, contact, industry_type FROM `$t`";
if ($locality !== '') {
...
<link rel="canonical" href="https://mycovai.in/industries" />
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $nm = $row["industry_name"];
        $slugBase = strtolower(preg_replace('/[^a-zA-Z0-9]+/','-',$nm));
        $slugBase = trim($slugBase, '-');
        $detail = '/industries/' . $slugBase . '-' . (int)$row['slno'];
        ...
        echo "<br><span class='muted-note'>" . $row["address"] . "</span>";
        echo "</div>";
